...
<div class="p-6">
  ...
  {{-- Tabs principales (los que ya venías armando) --}}
  <div class="mb-6 border-b border-gray-200">
    <nav class="flex gap-6">
      ...
      @foreach($tabs as $key => $label)
        <a href="{{ route('admin.reportes.index', ['tab' => $key, 'tipo' => $tipo, 'fecha' => $fecha]) }}"
           class="pb-3 border-b-2 {{ $tab === $key ? 'border-amber-500 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
          {{ $label }}
        </a>
      @endforeach
    </nav>
