...
<div class="container mx-auto px-4 py-6">
    ...
    @endif
        ...
        <div class="productos-table hidden md:block">
                            ...
                            @forelse($productos as $p)
                                <tr class="bb-row">
                                        ...
                                        <div class="flex items-center gap-3">
                                            @if(!empty($p->imagen))
                                                <div class="w-30 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm shrink-0">
                                                    <img src="{{ asset('storage/' . $p->imagen) }}" alt="Foto del producto" class="w-full h-full object-cover" loading="lazy">
                                                </div>
                                            @else
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full {{ ($p->estado ?? 'activo') == 'activo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            {{ ucfirst($p->estado ?? 'activo') }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.productos.show', $p->id) }}" class="bb-action inline-flex items-center justify-center leading-none text-gray-700 dark:text-gray-200" title="Ver">
                                            ...
                                            <a href="{{ route('admin.productos.edit', $p->id) }}" class="bb-action bb-action-edit inline-flex items-center justify-center leading-none" title="Editar">
                                                <svg class="w-4 h-4 block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <form action="{{ route('admin.productos.destroy', $p->id) }}" method="POST" class="m-0 p-0 inline-flex items-center" onsubmit="return confirm('¿Eliminar este producto?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="bb-action bb-action-del inline-flex items-center justify-center leading-none" title="Eliminar">
                                                    <svg class="w-4 h-4 block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- MOVIL: CARDS -->
        <div class="productos-cards space-y-3 md:hidden mt-6">
            @forelse($productos as $p)
                    ...
                    <div class="flex items-start gap-3">
                        @if(!empty($p->imagen))
                            <div class="w-20 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm shrink-0">
                                <img src="{{ asset('storage/' . $p->imagen) }}" alt="Foto del producto" class="w-full h-full object-cover block" loading="lazy">
                            </div>
                        @else
                            <span class="bb-icon-pill shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-xl leading-none">
                                <i class="fas fa-box text-base leading-none" style="color: rgba(201,162,74,.92)"></i>
                            </span>
                        @endif
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="text-xs text-gray-600 dark:text-gray-400 mt-2">
                                <span class="font-bold bb-gold">${{ number_format((float)$p->precio, 2) }}</span>
                        ...
                        <div class="flex flex-col items-end gap-2">
                            <a href="{{ route('admin.productos.show', $p->id) }}" class="bb-action inline-flex items-center justify-center text-gray-700 dark:text-gray-200" title="Ver">
                                <svg class="w-4 h-4 block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                            </a>
                            <a href="{{ route('admin.productos.edit', $p->id) }}" class="bb-action bb-action-edit inline-flex items-center justify-center" title="Editar">
                                <svg class="w-4 h-4 block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l9.586-9.586z"/>
                                </svg>
                            </a>
