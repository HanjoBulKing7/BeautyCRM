@extends('layouts.app')
@section('title','Editar Producto - Salón de Belleza')
    ...
    <div class="p-6" style="background: linear-gradient(135deg, rgba(201,162,74,.14), rgba(255,255,255,.78)); border-bottom: 1px solid rgba(201,162,74,.18);">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-gray-600 text-sm">Actualiza la información de {{ $producto->nombre }}</p>
            </div>
        </div>
    </div>

    <div class="p-6">
        <form method="POST" action="{{ route('admin.productos.update', $producto->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
