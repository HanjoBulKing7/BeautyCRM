                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="id_cliente" id="id_cliente" required
                                    ...
                                    @foreach($clientes as $cliente)
                                        <option value="{{ $cliente->id }}" {{ old('id_cliente', $cita->id_cliente) == $cliente->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                @php
                                    // ============================
                                    // Servicios extra para EDITAR
                                    // - Servicio principal (legacy): id_servicio
                                    // - Servicios extra: id_servicios[]
                                    // ============================
                                    $primaryId = old('id_servicio', $cita->id_servicio);
                                    $existingExtras = [];
                                    try {
                                        foreach (($cita->servicios ?? []) as $s) {
                                            $sid = $s->id_servicio;
                                            if ((string)$sid !== (string)$primaryId) $existingExtras[] = $sid;
                                        }
                                    } catch (\Throwable $e) {
                                        $existingExtras = [];
                                    }
                                    $oldExtras = old('id_servicios', $existingExtras);
                                    $oldExtras = array_values(array_filter((array) $oldExtras));
                                @endphp
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text"
                                           ...
                                           value="{{ old('fecha_cita', \Carbon\Carbon::parse($cita->fecha_cita)->format('Y-m-d')) }}">
                                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="hora_cita" id="hora_cita" required
                                        ...
                                        @php
                                            $horaSelected = old('hora_cita', \Carbon\Carbon::parse($cita->hora_cita)->format('H:i'));
                                            for($hora = 8; $hora <= 20; $hora++) {
                                                for($minuto = 0; $minuto < 60; $minuto += 30) {
                                                    $time = sprintf('%02d:%02d', $hora, $minuto);
                                                    $display_time = date('g:i A', strtotime($time));
                                                    echo "<option value=\"$time\" " . ($horaSelected == $time ? 'selected' : '') . ">$display_time</option>";
                                                }
                                            }
                                        @endphp
                                    </select>
                                    ...
                                    <div class="bg-white rounded-lg p-3 text-center">
                                        ...
                                        <p class="font-bold text-blue-700 flex items-center justify-center gap-2">
                                            <input
                                                ...
                                                id="duracion-input"
                                                @if(old('duracion_total_minutos') || ($cita->duracion_total_minutos ?? null)) data-manual="1" @endif
                                                value="{{ old('duracion_total_minutos', $cita->duracion_total_minutos ?? 0) }}"
                    ...
                    <div class="mt-10 pt-6 border-t border-gray-200">
                        <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4">
                            ...
                            <button type="submit"
                                ...
                                <i class="fas fa-calendar-plus mr-2"></i>Actualizar Cita
                            </button>
                        </div>
                    </div>
