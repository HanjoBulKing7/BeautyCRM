            ...
            <h1 class="text-2xl font-black text-gray-900 dark:text-white flex items-center gap-3">
                <div class="w-12 h-12 rounded-full bg-[rgba(201,162,74,0.1)] text-[rgba(201,162,74,1)] flex items-center justify-center shadow-sm shrink-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V6a2 2 0 012-2h4a2 2 0 012 2v1m-9 0h10a2 2 0 012 2v9a2 2 0 01-2 2H6a2 2 0 01-2-2V9a2 2 0 012-2zm0 6h10"/>
                    </svg>
                </div>
                Equipo y Personal
            </h1>
            <p class="text-sm text-gray-500 mt-1 ml-15">Gestiona a los empleados, sus datos de contacto y roles en el salón.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-gray-200/60">
                        <th class="pb-4 px-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Empleado</th>
                        ...
                        <tr class="hover:bg-white/40 transition-colors group">
                            ...
                            <td class="py-4 px-4 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('admin.empleados.edit', $empleado->id) }}" class="action-btn text-[rgba(201,162,74,1)] bg-[rgba(201,162,74,.1)] hover:bg-[rgba(201,162,74,.2)]" title="Editar">
                                    ...
                                    <form action="{{ route('admin.empleados.destroy', $empleado->id) }}" method="POST" class="inline-block m-0 p-0" onsubmit="return confirm('¿Seguro que deseas eliminar a este empleado del sistema?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-12 text-center">
                                ...
                                <h3 class="text-lg font-bold text-gray-900">No hay empleados registrados</h3>
                                <p class="text-gray-500 mt-1">Comienza agregando a tu primer colaborador.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="md:hidden space-y-4">
            ...
            <div class="bb-glass-card rounded-[2rem] p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-base font-black text-gray-900 truncate">{{ $empleado->nombre ?? $empleado->name ?? 'Empleado' }}</h3>
                        <div class="text-[10px] text-gray-400 uppercase tracking-wider mt-0.5 mb-2">ID: #{{ $empleado->id ?? '-' }}</div>
                ...
                <div class="flex justify-end gap-2 mt-4 pt-4 border-t border-gray-100/60">
                    <a href="{{ route('admin.empleados.edit', $empleado->id) }}" class="action-btn text-[rgba(201,162,74,1)] bg-[rgba(201,162,74,.1)] hover:bg-[rgba(201,162,74,.2)]">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l9.586-9.586z"/></svg>
                    </a>
                    <form action="{{ route('admin.empleados.destroy', $empleado->id) }}" method="POST" class="inline-block m-0 p-0" onsubmit="return confirm('¿Seguro que deseas eliminar a este empleado del sistema?');">
                        @csrf
        ...
        @empty
            <div class="bb-glass-card rounded-[2.5rem] p-10 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-900">No hay empleados</h3>
                <p class="text-gray-500 mt-1">Tu personal aparecerá aquí.</p>
            </div>
        @endforelse
