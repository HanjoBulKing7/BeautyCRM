@props([
    ...
    // Stats: Separamos el prefijo (+) del número real para poder animarlo
    'stats' => null,
])

@php
    $stats = $stats ?? [
        ['icon' => 'sparkle', 'prefix' => '+', 'value' => 1200, 'label' => 'Servicios Realizados'],
        ['icon' => 'heart',   'prefix' => '+', 'value' => 850,  'label' => 'Clientes Satisfechos'],
        ['icon' => 'badge',   'prefix' => '+', 'value' => 5,    'label' => 'Años de Experiencia'],
    ];
@endphp

{{-- Agregamos un ID a la sección para que JavaScript la encuentre --}}
<section id="stats-counter-section" class="relative bg-white py-20 md:py-24">
            ...
            @foreach($stats as $s)
                <div class="text-center">
                    ...
                    <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-black/5 shadow-sm"
                        ...
                        @switch($s['icon'])
                            @case('sparkle')
                                <svg viewBox="0 0 24 24" class="h-9 w-9" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    {{-- Contenedor del número a animar --}}
